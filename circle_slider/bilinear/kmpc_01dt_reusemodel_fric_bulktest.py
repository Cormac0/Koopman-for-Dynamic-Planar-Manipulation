...
def main_script(mu_t=10., SAVE=False, PLOT=False, init_config = None):
    ...
    checkpoint_stab = torch.load('nn_multistep_loss_1/model.pt')
    #checkpoint_stab = torch.load('model.pt')
    #checkpoint_stab = torch.load('models/bigger_size_nn_multistep_loss_1.pt') # works?
    #checkpoint_stab = torch.load('variable_mu/nn_domainrand/model.pt')
    # checkpoint_stab = torch.load('models/nn_multistep_loss_1.pt') # works?
    # checkpoint_stab = torch.load('models/nn_multistep_loss_full_traj.pt') # fails
    # checkpoint_stab = torch.load('models/nn_multistep_loss_biggerdatadomain.pt') # fails
    # checkpoint_stab = torch.load('models/nn_multistep_loss_1_archive2.pt') # fails
    # checkpoint_stab = torch.load('models/nn_multistep_loss_1_archive.pt') # fails
    ...
    for mu_count, mu_t in enumerate(test_mu):
        for init_count, test_yerr in enumerate(test_yerr_arr):
            for p2_count, test_p2_val in enumerate(test_p2):
                ...
                results = main_script(mu_t=mu_t, SAVE=False, PLOT=True, init_config=init_config)
                results_arr.append(results)
    # Save the results
    #np.save('results/og_fric_results.npy', results_arr)
