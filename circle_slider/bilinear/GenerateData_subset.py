...
def generate_data(conduit_traj, max_energy,traj_len,pusher,save_path,num_states,dt, domains, SAVE=False,VERBOSE=False, duration=1.0,seed=0, file_array_size=10000,file_count=0):
    ...
    def zoh_control(t):
            idx = np.searchsorted(ctrl_times, t, side='right') - 1
            idx = np.clip(idx, 0, len(ctrl_traj)-1)
            return ctrl_traj[idx]

    def dynamics_zoh(t, state):
        ctrl = zoh_control(t)
        return pusher.dynamics_bodyu(t, state, ctrl)
    ...
    while i <= file_array_size:
        ...
        ctrl_traj = rng.uniform(low=[-1.*0.01,-50.*0.01], high=[1.*0.01,50.*0.01], size=(traj_len, 2)) # shape: (traj_len, control_dim)
        ctrl_times = np.linspace(0, traj_len*dt, traj_len)
        ...
        MAX_REDO = 50
        if IN_CONTACT:
            px = rng.uniform(low=min_pbody1, high=pusher.block_diameter/2,size=None)
            state0[2] = px
        else:
            px = rng.uniform(low=pusher.block_diameter/2, high=max_pbody1,size=None)
            state0[2] = px
        #ENERGY = 0.5*ring_shaft.ring_mass*(np.linalg.norm(state0[6:9]))**2
        if VERBOSE:
            print('State0: ' + str(state0))
        #results = spi.odeint(ring_shaft.dynamics, state0, np.linspace(0,traj_len*dt,traj_len))
        ...
        results_full_ivp = spi.solve_ivp(dynamics_zoh, method='LSODA',t_span=(0,traj_len*dt),t_eval=ctrl_times, y0=state0)
        #sample_time_end = time.time()
        #sample_time = sample_time_end - sample_time_start
        results = results_full_ivp.y.T
        # Concatenate control inputs to results
        results = np.hstack([results, ctrl_traj])  # shape: (traj_len, state_dim + control_dim)
        #print('Elapsed time for sample ' + str(i) + ': ' + str(sample_time) + ' seconds')
        # if sample_time > 1:
        #     print('State:',state0)
        #     print("Results:",results_full_ivp.y[:,-1])
        ...
        if i % num_subsamples == 0:
            ...
            prev_time = time.time()
            # if len(data) >= file_array_size:
            #     if SAVE:
            #         np.save(save_path+'/Fx_'+str(traj_len)+'_'+str(len(data))+'_'+str(seed)+'_file'+str(file_count)+'.npy',data)
    ...
    print('Seed:',seed)
    #data_arr[count_x0,count_y0,count_z0,count_psi0,count_theta0,count_phi0,count_xdot0,count_ydot0,count_zdot0,count_psidot0,count_thetadot0,count_phidot0,:,:] = results
    if SAVE:
...
if __name__ == '__main__':
    ...
    domains = [x_domain, y_domain, pbody1_domain, pbody2_domain, x_dot_domain, y_dot_domain]

    dt = 0.01
    conduit_traj = np.load('pusher_trajectory.npy')
    init_state = np.array(
        [0, 0, -block_length, 0, 0, 0]
