...
class L3():
	...
	def step(self, Fx_arr_batch: torch.Tensor, model: torch.nn.Module, loss_fn: Callable):
		...
		time_weight = 0.95
		for time_step in range(Fx_arr_batch.shape[1]-2):
		#for time_step in range(3):
			#x_hat, eta_hat = model(x_hat)
			x_hat, eta_hat = model.ldm(xi_hat)
			xi_hat = torch.cat((x_hat, eta_hat), 1)
			multi_step_loss_state += time_weight**(time_step+1)*loss_fn(Fx_arr_batch[:, time_step+2, :], x_hat)
			multi_step_loss_eta += time_weight**(time_step+1)*loss_fn(model.enc(Fx_arr_batch[:, time_step+2, :]), eta_hat)
